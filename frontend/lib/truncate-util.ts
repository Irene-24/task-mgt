...
function stripEscapedCharacters(str: string): string {
  ...
  const strippedString = str.replaceAll(escapedCharsRegex, "").trim();
 ...
 *
 * @param {React.RefObject<HTMLElement | null>} elementRef - The React ref object that points to the target element.
...
const getElementComputedStyles = (
  ...
  if (elementRef.current) {
    const computedStyle = globalThis.getComputedStyle(elementRef.current);

    return Number.parseFloat(computedStyle[property] || "0");
  }
